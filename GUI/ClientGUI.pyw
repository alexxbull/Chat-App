...
def send_message(event = None, textBox = sendTextBox):
    ...
    try:
        ...
        if text != "":
            client.send(text)
        return "break"                                                              # prevents <Return> from inserting a new line so cursor in text box returns the first line rather than second after hitting send
    except socket.error as err:
        printUI("Lost connection with server.\nExiting...")
        time.sleep(2)
        quit()
...
def make_chatGUI():
    ...
    chatText.grid(sticky = tk.N + tk.S + tk.E + tk.W)                               # make the chat text log read only
    ...
    sendButton = tk.Button(chatFrame, text = "Send", width = 5, height = 1, command = send_message, bg = "#2d89ef", fg = "white")
    sendButton.grid(row = 2, column = 0, sticky = tk.N + tk.S + tk.E + tk.W)        # expand button in center button
...
def start_GUI():
    ...
    initialFrame.grid()
    rt = threading.Thread(target = join_server)                                     # make a new thread for handling the client communicating with the server
    rt.start()
    ui.mainloop()
...
"""
Active Threads:
- Main/GUI thread will handle sending messages to server
- A new thread (rt) is made for joining and receiving message from server
"""
